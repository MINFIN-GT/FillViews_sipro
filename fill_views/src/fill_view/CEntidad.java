		...
		try{
			if( !conn.isClosed() && CMariaDB.connect()){
			   ...
			   pstm1.executeUpdate();
				...
				pstm1 = CMariaDB.getConnection().prepareStatement("Delete from sipro.cooperante where ejercicio=" + ejercicio);
				pstm1.executeUpdate();
				pstm1.close();
				
				pstm1 = CMariaDB.getConnection().prepareStatement("Insert INTO sipro.cooperante (codigo,siglas,nombre,usuario_creo,fecha_creacion,estado,ejercicio) "
						+ "values (?,?,?,?,?,?,?) ");
